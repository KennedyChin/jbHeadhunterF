...
export class UploadResumeComponent implements OnInit {
    ...
    if (file) {
      ...
      if (size >= 10485760) {
        ...
        upload$.subscribe({
          next: (response: any) => {
            // alert('檔案正確');
            this.uploadSub = upload$.subscribe((n: any) => {
              this.fileKeyID = n.fileUploadKey;
            });
          },
          error: (error: any) => {
            if (error.status === 400) {
    ...
    if (this.resumeInput?.nativeElement.files?.length === 0) {
      ...
      if (!this.agreePrivacyPolicy) {
        ...
        if (form.valid) {
          ...
          const sendUserData$ = this._http
            .post(`${environment.apiUrl}/File/UpdateFileInfo`, jsonBody, {
              headers: headers,
            })
            .pipe(finalize(() => {}));
          ...
          sendUserData$.subscribe({
            ...
            error: (error: any) => {
              if (error.status === 404) {
                alert(
                  '找不到對應資訊，請檢查欄位及履歷是否均有填寫，並請於稍後重試。'
                );
              } else {
                alert('連線異常，請稍後再試');
              }
  ...
  backToMain() {
    ...
    // 將使用者重新導回首頁
    window.location.href = environment.homePage; // 將 URL 設為首頁的 URL
  }

  openPrivacyPolicy() {
    window.open(`${environment.homePage}/Privacy`, '_blank');
  }
