...
export class JobsListComponent implements OnInit {
  ...
  getMeInfo(placeId: number, jobId: number, t: string | null) {
    let o = !!t
      ? this._jobService.getSearchJobs(placeId, jobId, t)
      : this._jobService.getSearchJobs(placeId, jobId, ' ');
    this.jobs$ = o;
  }
  ...
  queryParamSubscribe() {
    ...
    if (this.userNo === 0) {
      this._route.queryParamMap.subscribe((n) => {
        ...
        if (id !== null) {
          this.jobs$ = this._jobService.getJobDetail(+id).pipe(
            first(),
            map((o) => [o])
          );
          return;
